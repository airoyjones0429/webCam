...
app.use('/hls', express.static(path.join('E:', 'hls')));
...
app.get('/hls/output:dynamicId.m3u8', (req, res) => {
    ...
    // 年月日時分秒.m3u8
    const filePath = path.join('E:', 'hls', `output${dynamicId}.m3u8`); 
...
app.get('/hls/:segment', (req, res) => {
    ...
    const filePath = path.join('E:', 'hls', req.params.segment); 
    // 設定 MIME 類型
...
app.get('/api/hls-files', (req, res) => {
    const hlsDirectory = path.join('E:', 'hls');
...
function getNewOutputFileName()  {
    ...
    // 生成新的檔案名稱
    const  outputFileName1 = path.join('E:', 'hls', `output_${formattedDate}.m3u8`) ; 
...
function startRecording() {
    ...
    ffmpeg = spawn(ffmpegPath, [
        ...
        '-i', 'video=@device_pnp_\\\\?\\usb#vid_1908&pid_2311&mi_00#7&c871309&0&0000#{65e8773d-8f56-11d0-a3b9-00a0c9223196}\\global',
        '-f', 'dshow',
        ...
        '-framerate', '15', // 幀率
        '-hls_time', '5', // 每個 HLS 片段的持續時間  設定 5 秒長度
